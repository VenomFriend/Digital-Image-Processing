...
def main():
    ...
    imgOriginal = Image("Image_(4).jpg")
    imgFiltered = Image("resultadoPeso_7x7_5vezes_Image_(4).jpg")

    hist = Histogram()
    h_original = hist.getValues(imgOriginal.arr)
    h_filtered = hist.getValues(imgFiltered.arr)
    h_diff = hist.diff(h_original, h_filtered)
    hist.save(h_filtered, imgFiltered.name, "h_filtered")
    hist.save(h_diff, imgFiltered.name, "h_diff")
